...
**Source Recording**:
The immutable local video file from which Clip Proposals and Accepted Clips are derived.
_Avoid_: VOD URL, downloaded asset, working copy

**Source Time**:
Decimal seconds from the playable beginning of the original Source Recording, where that beginning is 0.0. It is the sole time coordinate used for all editorial concepts.
...
**Work Directory**:
The repository-local, Git-ignored `workdir/` that contains every downloaded model and runtime, private Source Recording, database, generated artifact, cache, log, temporary file, and export.
...
**MTP Drafter**:
The Multi-Token Prediction component used for speculative decoding. It is embedded in the selected Qwen3.6 GGUF and stored as a separate GGUF beside the Gemma 4 target.
_Avoid_: Small model, fallback model, second evaluator

**GPU Lease**:
Exclusive permission for one disposable worker process tree to own CUDA resources until that tree has exited and GPU memory release has been verified.
...
**Reference Highlight**:
A manually annotated Highlight used to evaluate retrieval and boundary quality, marked as definite or possible with a category and ideal start and end.
_Avoid_: Gold label, test candidate
...
**Clip Proposal**:
A machine-suggested interval from a Source Recording, including proposed boundaries and the evidence for reviewing it. It has not yet been approved for export.
...
**Editorial Decision**:
The editor's accept, maybe, or reject judgment on a Clip Proposal, including any correction to its boundaries.
_Avoid_: Model label, prediction

**Rejection Reason**:
The editor's primary explanation for rejecting a Clip Proposal: no payoff, too much required context, routine content, repetition, poor media quality, poor Creator Fit, publication Risk, or wrong boundaries.
_Avoid_: Feedback, note, model rationale

**Boundary Edit**:
The editor's correction to a Clip Proposal's proposed start or end. It records how much Setup and Payoff the creator actually wanted.
_Avoid_: Trim, timestamp fix

**Accepted Clip**:
A source-aspect video rendered from the original Source Recording after its Clip Proposal is accepted and, when necessary, trimmed.
_Avoid_: Auto-generated highlight
...
**Risk**:
A potential reason not to publish a Clip Proposal, including privacy, hateful language, copyrighted material, or exposed personal information.
_Avoid_: Low quality, rejection reason
