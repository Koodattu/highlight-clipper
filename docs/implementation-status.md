...
| Transcript retrieval | Lexical windows plus Qwen3-Embedding-0.6B in a serialized CUDA worker; explicit bfloat16/SDPA execution, hard CUDA placement checks, versioned `.npy` artifact, and device-aware lineage | Prior CPU embedding worker completed a real run; CUDA adapter policy has offline coverage | CUDA real-model smoke plus held-out recall/latency ablation; BGE-M3 is download-only, not an analysis adapter |
...
| GPU lifecycle | Application-wide Windows named mutex, Job Object ownership, one model process at a time; ASR, CUDA embeddings, and fully offloaded llama.cpp execute sequentially; worker and VRAM telemetry are persisted where available | Real ASR/evaluator runs left no owned model process; offline embedding tests require the GPU worker path | CUDA embedding smoke plus representative cancellation/crash and external-VRAM-pressure testing; named-mutex owner, peak VRAM, and post-unload llama VRAM are not persisted |
